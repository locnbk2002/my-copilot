...
Invoke `test` skill to run the project test suite.
...
Invoke `fix` skill with the failing test output as context.
- Max 2 fix attempts
...
Dispatch `task(agent_type="code-reviewer")` targeting all files changed in this execution.
...
Invoke `docs` skill if implementation changed APIs, interfaces, or public behavior.
...
Invoke `git cm --atomic` for phase-aware atomic commits.
