...
Category-to-model mapping used by `worker` to delegate plan phases to sub-agents.
...
5. Dispatch: `task(agent_type=resolved_agent, model=resolved_model, mode="background", prompt=phase_content)` → get `agent_id`, then `read_agent(agent_id, wait=True, timeout=300)`

## Category Auto-Tagging Rules (for plan)
