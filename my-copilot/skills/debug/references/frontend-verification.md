...
Visual verification of frontend implementations using Chrome MCP (Claude Chrome Extension) or `chrome-devtools
> Note: `chrome-devtools` is not yet available in this plugin.` skill fallback.
...
Check if Chrome MCP server is available via `mcp-management
> Note: `mcp-management` is not yet available in this plugin.` skill or `ListMcpResourcesTool`:
