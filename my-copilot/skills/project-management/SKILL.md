...
> ⚠️ **Compatibility Note:** This skill references Task management APIs (TaskCreate, TaskUpdate, TaskGet, TaskList)
> that may not be available in all copilot-cli contexts. Verify your runtime supports these tools before use.
...
- **Sync-ba** Reconcile all completed tasks against all phase files, update `[ ]` → `[x]`, update YAML frontmatter status
