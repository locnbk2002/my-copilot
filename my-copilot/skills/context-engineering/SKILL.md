...
Context window info available via copilot-cli:

- **`context-reporter` extension** (auto): real `<usage-awareness>` injected after each model call — exact K/K tokens from `session.usage_info`
- **statusLine `context` module** (requires `--experimental`): real-time % in status bar
- **`/context` slash command**: on-demand context visualization

**Thresholds:**

- 70%: WARNING — plan compaction strategy
- 80%: Execute compaction
