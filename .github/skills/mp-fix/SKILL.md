...
```
mp-fix [description] [--quick] [--file path]
...
## When to Use
- After test failures (standalone or invoked by mp-test)
- When error messages need diagnosis
- For debugging issues found during code review
- As part of mp-execute failure recovery
...
| Medium | Logic error, missing edge case | Use `mp-debugger` agent for root cause |
| Complex | Architectural issue, multi-file | Create mini-plan, fix systematically |

### 3. Root Cause (skip if `--quick`)
- Dispatch `task(agent_type="mp-debugger")` for root cause analysis
- Or use `mp-sequential-thinking` skill for step-by-step reasoning
...
- `mp-debugger` agent — Deep root cause analysis
- `mp-sequential-thinking` — Step-by-step reasoning for complex bugs
- `mp-test` — Invokes this skill via `--fix` flag
- `mp-execute` — Invokes this skill when verification fails
