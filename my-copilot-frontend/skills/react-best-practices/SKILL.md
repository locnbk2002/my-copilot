...
Apply these rules when writing React/Next.js code. Each rule ID maps to a specific pattern:
...
**Example — bundle-dynamic-imports:**

```typescript
// BAD: Eagerly imported heavy component
...
**Example — rerender-memo:**

```typescript
// BAD: Expensive work inside component
function List({ items }) {
  const sorted = items.sort((a, b) => b.date - a.date) // re-runs every render
...
**Example — rendering-conditional-render:**

```typescript
// BAD: && with non-boolean (renders "0")
